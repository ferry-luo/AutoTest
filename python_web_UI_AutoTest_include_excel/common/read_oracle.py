...
class DB:
    ...
    #查询
    def select(self):
        query_sql = "select product_name from dl_product where product_name LIKE 'Swisse%' AND ROWNUM <= 50"
        with self.connection.cursor() as x:
            x.execute(query_sql)
            result = x.fetchall()   #返回多个元组，也即返回多条记录rows
            print(result)
            for i in result:
                #print(i)
                for j in i:
                    ...
                    arr = []
                    arr.append(j)
                    if arr[0] != "Swisse ACAI BERRY SUPERFOOD POWDER 50G":
                        print("不是第一个产品名称")
    #定义 清除表中数据 的函数
...
if __name__ == "__main__":
    ...
    data = {"serial_code": "34343434343434", "midcode": "0", "big_code": "555555555555555555555555", "product_id": 857,"serial_type": 3, "dealer_id": 100603, "batch": "6142201808142", "storehouse_code": 1000,"upload_time": "to_date('2018-08-15 09:30:20','yyyy-mm-dd hh24:mi:ss')", "position": 2, "status": 2,"split_type": 0, "delivery_Type": 1, "destination_id": 100603, "destination_name": "杭州黄浦贸易有限公司","destination_type": 3, "destination_time": "to_date('2019-01-24 09:16:10','yyyy-mm-dd hh24:mi:ss')"}
